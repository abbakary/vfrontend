...
export default function AdminFundRequestsPage({ role = "admin" }) {
  ...
  const load = () => setRequests(fundRequestService.getAllRequests());
  ...
  const approve = () => {
    if (!selected) return;
    fundRequestService.approveRequest(selected.id, { reviewerRole: role, reviewNotes });
    setOpen(false);
    setSelected(null);
    load();
  };
  ...
  const reject = () => {
    if (!selected) return;
    fundRequestService.rejectRequest(selected.id, { reviewerRole: role, reviewNotes });
    setOpen(false);
    setSelected(null);
    load();
  };
  ...
  const cancel = (id) => {
    fundRequestService.cancelRequest(id, { reviewedByRole: role });
    load();
  };
      ...
      <Box sx={{ backgroundColor: themeColors.bg, transition: "background-color 0.3s ease" }}>
        <Box sx={{ mb: 4 }}>
          ...
          <Typography sx={{ color: themeColors.textMuted, fontSize: "1rem" }}>
            Review funding requests submitted from the public Funds page
          </Typography>
        </Box>
        ...
        {filtered.length === 0 ? (
          <Card sx={{ borderRadius: 2, border: `1px solid ${themeColors.border}`, boxShadow: "none", p: 4, textAlign: "center", backgroundColor: themeColors.card }}>
            <Typography sx={{ color: themeColors.textMuted, fontWeight: 700 }}>No fund requests found</Typography>
          </Card>
        ) : (
                    ...
                    <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 2 }}>
                      <Box>
                        <Typography sx={{ fontSize: "1.1rem", fontWeight: 800, color: themeColors.text }}>
                          {r.title} • {r.fundingType}
                        </Typography>
                        <Typography sx={{ fontSize: "0.85rem", color: themeColors.textMuted, mt: 0.5 }}>
                          {r.userName} • {r.userEmail} • {r.company}
                        </Typography>
                        <Typography sx={{ fontSize: "0.85rem", color: themeColors.textMuted, mt: 0.5 }}>
                          Amount: <b style={{ color: themeColors.text }}>{r.amountCurrency} {Number(r.amount || 0).toLocaleString()}</b> • Timeline: {r.timeline}
                        </Typography>
                      </Box>
        ...
        <Dialog open={open} onClose={() => setOpen(false)} maxWidth="sm" fullWidth PaperProps={{ sx: { backgroundColor: themeColors.card } }}>
          {selected && (
            <>
              <DialogTitle sx={{ fontWeight: 900, color: themeColors.text }}>Review fund request</DialogTitle>
              <DialogContent sx={{ color: themeColors.text }}>
                <Box sx={{ display: "grid", gap: 1.5, mt: 1 }}>
                  <Card sx={{ borderRadius: 2, border: `1px solid ${themeColors.border}`, boxShadow: "none", backgroundColor: themeColors.bgSecondary }}>
                    <CardContent sx={{ p: 2 }}>
                      <Typography sx={{ fontWeight: 900, color: themeColors.text }}>{selected.title}</Typography>
                      <Typography sx={{ color: themeColors.textMuted, mt: 0.5 }}>
                        Type: {selected.fundingType} • Category: {selected.category} • Data: {selected.dataType}
                      </Typography>
                      <Typography sx={{ color: themeColors.textMuted, mt: 0.5 }}>
                        Amount: {selected.amountCurrency} {Number(selected.amount || 0).toLocaleString()} • Timeline: {selected.timeline}
                      </Typography>
                      <Typography sx={{ color: themeColors.textMuted, mt: 0.5 }}>
                        Requested by: {selected.userName} • {selected.userEmail} • {selected.company}
                      </Typography>
